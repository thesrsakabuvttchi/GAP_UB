...
parser.add_argument('--filter-file', type=str, 
                    help='Name of the file we want to filter out',
                    default=None)
...
for (file_name, line_number), group in grouped:
    ...
    try:
        ...
        captured_data.append((file_name,line_number,results['branch-misses'],results["conditional-branch-misses"],results['br_inst_retired.cond'],results['branches'],round(miss_rate,2),group.shape[0]))
    except Exception:
        miss_rate = 0
        captured_data.append((file_name,line_number,results.get('branch-misses'),results.get("conditional-branch-misses"),results.get('br_inst_retired.cond'),results.get('branches'),round(miss_rate,2),group.shape[0]))

df = pd.DataFrame(captured_data, columns=['File', 'line_number', "branch_misses", "conditional_branch_misses","conditional_branches", "total_branches",'Misprediction rate', 'Sample_number'])
...
if(enable_mergable_data):
    ...
    print("{0} % of Conditional misses can be eliminated".format(miss_ratio))  
    print("_"*80)
    print()
